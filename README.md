...
1. ジャンクションを作成（管理者不要、初回のみ）。PowerShell の場合:

   ```powershell
   New-Item -ItemType Junction -Path "$env:APPDATA\Autodesk\Autodesk Fusion 360\API\AddIns\QuhpCamAssistant" -Target "E:\Projects\fusion-addin\QuhpCamAssistant"
   ```

   （`mklink /J` は cmd.exe 専用コマンドのため PowerShell では使えない）
...
2. スクリプトとして [tools/DumpParameters](tools/DumpParameters) を実行し、ダンプを保存
