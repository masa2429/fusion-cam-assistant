...
- 2026-07-16 ／ 負荷制御が細幅領域（幅<工具径×1.25）で垂直プランジする ／ adaptive2d の進入はヘリカル系のみ（profile/zigzag は API が拒否）で、成立しない幅ではプランジにフォールバックする。パラメータ調整では解決不可 ／ 分類側で成立幅を判定し、細幅ポケット面は輪郭パス（contour2d・プロファイルランプ・床基準0mm）へ自動振替（classifier.py `_group_pocket_faces`）。**/codex への相談で方針確定（a:振替 > b:境界拡張 > c:プランジ許容）**
