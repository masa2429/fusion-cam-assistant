...
- **リリース手順**: 実機検証が済んだら dev → main にマージし、`FusionCamAssistant.manifest` の `version` を上げ、`git tag vX.Y.Z` を打つ（更新通知は version 比較で飛ぶため、リリース時だけメンバーに通知される）
- **バージョン規則（X.Y.Z）**: リリース1回につき1箇所だけ上げる
  - **X（メジャー）**: メンバーに再インストールや手作業が必要な互換性のない変更（アドイン名/フォルダ変更、config 形式の非互換変更、テンプレ命名規約の変更など）
  - **Y（マイナー）**: 機能追加（新コマンド、新しい加工種別・テンプレセット対応、ダイアログの新機能）
  - **Z（パッチ）**: バグ修正・安全性修正・小さな改善（分類精度、進入方法の修正、文言など）
  - 1.0.0 の目安: 実切削での運用が安定し、テンプレ規約と config が固まったとき
...
- 2026-07-16 ／ 負荷制御/ポケットが部品端付近でストックに垂直進入する（ヘリカル可能な面でも発生） ／ 部品外形に開いた面はストック端との関係で進入の逃げ場が無い。adaptive2d の進入はヘリカル系のみ（profile/zigzag は API が拒否）でパラメータ調整では解決不可 ／ **開口辺を検出して境界スケッチを自動生成し、開口辺だけストック余白側へ拡張して渡す**（ユーザーが手作業でやっていた方式の自動化。cam_builder.py `_build_extended_boundary_sketch`）。閉じた細幅面のみ輪郭パスへ振替（classifier.py `_group_pocket_faces`）。/codex にも相談したが、**最終方針はユーザーの実務経験（境界拡張）を採用。実機で解消を確認済み**
- 2026-07-16 ／ 境界拡張スケッチの落とし穴3連発 ／ (1)面上にスケッチを作ると全エッジが自動投影され境界が二重になる→**同じ高さの構築平面に作る** (2)連続する開口辺は各辺ごとに元頂点へ戻すと領域が繋がらない→**オフセット線同士をマイター交点で接続** (3)スケッチ選択は既定だと外側にパスが出る→**サイド「内側から開始」とループ種別を API で明示設定**（穴ループは含めず外側のみ・島(凸)は投影して全ループ） ／ すべて cam_builder.py に実装。**実機で解消を確認済み**
- 2026-07-16 ／ ワークサイズが10mmの倍数のとき外郭パスが省略される ／ 固定ボックスストックの10mm切り上げでは余白がゼロになる場合があり、ストック＝ワークだと外郭の削り代が無い扱いになる ／ ストックを相対ボックス＋側面余白（stock_side_margin_mm、既定5mm）に変更（cam_builder.py）
...
- 2026-07-16 ／ WCS原点の角が意図（ストック上面・左下手前）と違う角になるモデルがある ／ `wcs_origin_boxPoint = 'top 1'` の番号と物理角の対応がモデル依存で不定 ／ bbox＋側面余白から角の座標を自前計算し、スケッチ点を `wcs_origin_mode='point'` ＋ `wcs_origin_point`（**リスト代入** `.value.value = [sketchPoint]`）で明示指定（cam_builder.py `_create_wcs_origin_point`）。'top N' 系はフォールバック専用。**実機で修正効果を確認済み**
- 2026-07-16 ／ 自動配置で `arrangeComponents.add` が「No fusion asset adapter from active asset adapter」 ／ アドインのボタンは製造ワークスペースにあり、CAM 環境がアクティブなまま設計フィーチャ（整列）を作ろうとした ／ 作成の間だけ `FusionSolidEnvironment` を activate して戻す（auto_place.py）。**設計フィーチャを作る API はアクティブなワークスペースに依存することがある**。実機で解消を確認済み
- 2026-07-17 ／ 自動配置で全部品が上下反転（鏡像加工事故のもと） ／ `arrangeComponents.add(occurrence)` の既定の向きが平板を裏返す。公式ドキュメントの BRepFace 渡し＋`isGlobalDirectionFaceUp` は実機では `InternalValidationError: arrange2DDefinition` で拒否される ／ オカレンス渡し＋各 `ArrangeComponent.isDirectionFlipped = True` で正立に戻し、配置後に `occurrence.transform2` の (2,2) 成分の符号で反転を自動検出して結果に表示（auto_place.py）。**実機で修正効果を確認済み**
- 2026-07-17 ／ 回転配置された部品でポケットの境界拡張が内側を向く（進入の逃げ場が消える） ／ 拡張方向を壁面 evaluator の法線に頼っていたが、evaluator の法線はプロキシ・面の表裏の文脈で反転することがある（実機ログで全開口辺が正反対を向いていた）。平行移動だけの配置では方向ベクトルが変わらないため潜伏していた ／ **外向きはループ自身の周回方向（符号付き面積）と頂点座標から決める**（cam_builder.py `_ordered_coedge_entries`）。頂点座標はプロキシでもワールドで信頼できる。**方向・位置は evaluator でなくデータ（頂点・bbox・周回方向）から導くこと**。実機で修正効果を確認済み
