...
- 2026-07-16 ／ 負荷制御/ポケットが部品端付近でストックに垂直進入する（ヘリカル可能な面でも発生） ／ 部品外形に開いた面はストック端との関係で進入の逃げ場が無い。adaptive2d の進入はヘリカル系のみ（profile/zigzag は API が拒否）でパラメータ調整では解決不可 ／ **開口辺を検出して境界スケッチを自動生成し、開口辺だけストック余白側へ拡張して渡す**（ユーザーが手作業でやっていた方式の自動化。cam_builder.py `_build_extended_boundary_sketch`）。閉じた細幅面のみ輪郭パスへ振替（classifier.py `_group_pocket_faces`）。/codex にも相談したが、**最終方針はユーザーの実務経験（境界拡張）を採用**
