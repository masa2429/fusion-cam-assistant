...
- ❗ **evaluator（`face.evaluator` / `edge.evaluator`）の返す方向・位置は文脈依存で信用しない**
  （実機確認 2026-07-17）: プロキシ（オカレンス文脈）や面の表裏（isParamReversed）で法線が
  反転・未変換になることがある。回転配置の部品で壁面の「外向き」法線が全て内側を向いた実例あり。
  - ワールドの**方向**が要るとき: 頂点座標（`edge.startVertex.geometry` はプロキシでもワールド ✅）
    とループの周回方向（符号付き面積）から導く（cam_builder.py `_ordered_coedge_entries`）
  - ワールドの**位置**が要るとき: `boundingBox`（プロキシでもワールド ✅）か、
    nativeObject で評価して `assemblyContext` の transform を明示適用（fusion_utils.proxy_world_transform）
  - 向き・面積・周長など**剛体変換に不変な量**の計算は同一空間内で完結していれば evaluator でも安全
  - `normal.z` による上下判定（水平面）は Z 軸回転の影響を受けないため従来どおり可
...
- **ボタンアイコンのダークテーマ対応 ✅（実機確認済み 2026-07-18）**: `addButtonDefinition` に渡すリソースフォルダに `16x16-dark.png` / `32x32-dark.png` / `64x64-dark.png` を置くと、**ダークテーマ時に Fusion が自動で拾う**（サフィックス無しがライト用）。アドイン側のコードでテーマを判定する必要はない。他に `-disabled` / `@2x`（HiDPI）のサフィックスも同様に効く。
  確定ソース: Fusion 本体のリソース実物（`%LOCALAPPDATA%\Autodesk\webdeploy\production\<hash>\Animation\UI\AnimationUI\Resources\Assembly\ExplodeAllParts\` に `32x32.png` と `32x32-dark.png` が対で存在）。公式ドキュメントには記載が見当たらない。
  ダーク版は配色を明るい側へ振る（本プロジェクトの配色は tools/dev/make_icons.py の `PALETTES` 参照）。グリフ上に白で「くぼみ」を描く表現はダークでは沈むので背景寄りの暗色に差し替えること
