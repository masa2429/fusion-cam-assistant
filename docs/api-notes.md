...
## 7. 整列（Arrange）API ✅（クラス・シグネチャ確定 2026-07-16、動作は⚠️実機未確認）
...
- ❗ `arrangeComponents.add(occurrence)` は「**最大の面が下向き**」になるよう部品を向ける仕様。
  平板は上下面が同面積なので**上下反転（鏡像加工事故）の可能性を実機確認すること**。
  BRepFace を渡すと面の向き基準になる（`definition.isGlobalDirectionFaceUp` と併用）
- ⚠️ 実機未確認: `add()` の成否（トゥルーシェイプが拡張なし環境で通るか）、平面 envelope の
  原点位置（構築平面原点＝枠の角か中心か）、`quantity` への代入型、部品の上下反転の有無
